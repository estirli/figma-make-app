...
function figmaSiteConfiguration(config: FigmaSiteConfiguration): Plugin {
  ...
  return {
    ...
    configureServer(server) {
      // Dev proxy for /api/proxy — mirrors the Vercel Edge Function in production
      server.middlewares.use('/api/proxy', async (req, res) => {
        try {
          ...
          const upstream = await fetch(target, { headers: { Accept: 'application/json', 'User-Agent': 'Mozilla/5.0' } })
